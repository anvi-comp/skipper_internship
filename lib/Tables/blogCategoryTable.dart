...
class blogCategoryTable extends StatefulWidget {
  const blogCategoryTable({Key? key, required String title}) : super(key: key);

  @override
  _DocumentsPageTableState createState() => _DocumentsPageTableState();
}

class _DocumentsPageTableState extends State<blogCategoryTable> {
  final List<Map> _desData = List.generate(5, (i) {
    return {
      "id": i,
      "Name of Blog Category": "Dept $i",
      "Description": "Desc $i",
      "Status": "Active",
      "columnEdit": false
    };
            ...
            rows: _desData.map((item) {
              return DataRow(cells: [
                DataCell(_isindex == item["id"]
                    ...
                    : Text(item['Name of Blog Category'])),
                DataCell(_isindex == item["id"]
                    ? TextFormField(
                    initialValue: item['Description'],
                    style: TextStyle(fontSize: 14))
