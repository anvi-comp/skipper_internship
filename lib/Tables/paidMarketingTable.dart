...
class paidMarketingTable extends StatefulWidget {
  const paidMarketingTable({Key? key, required String title}) : super(key: key);

  @override
  _DocumentsPageTableState createState() => _DocumentsPageTableState();
}

class _DocumentsPageTableState extends State<paidMarketingTable> {
  final List<Map> _desData = List.generate(5, (i) {
    return {
      "id": i,
      "Name of the Paid Marketing": "Dept $i",
      "Description": "Desc $i",
      "Status": "Active",
      "columnEdit": false
    };
            ...
            rows: _desData.map((item) {
              return DataRow(cells: [
                DataCell(_isindex == item["id"]
                    ...
                    : Text(item['Name of the Paid Marketing'])),
                DataCell(_isindex == item["id"]
                    ? TextFormField(
                    initialValue: item['Description'],
                    style: TextStyle(fontSize: 14))
