...
class kpiPageList extends StatelessWidget {
  const kpiPageList({Key? key}) : super(key: key);

  static Route route() {
    return MaterialPageRoute<void>(
        builder: (_) => BlocProvider(
          create: (_) => DocumentsBloc(),
          child: const kpiPageList(),
        ));
  }
...
class kpiPageTable extends StatefulWidget {
  const kpiPageTable({Key? key}) : super(key: key);

  @override
  _kpiPageTableState createState() => _kpiPageTableState();
}

class _kpiPageTableState extends State<kpiPageTable> {
  ...
  getData() async {
    var _data = await httpService.getData("kpi");
            ...
            rows: _desData.map((item) {
              return DataRow(cells: [
                ...
                DataCell(_isindex == item["id"]
                  ...
                  httpService.updateData("kpi", _isindex, item);
                  getData();
                  setState(() {
                    _isindex = -1;
                  });
                  ...
                  onRowClick: (bool edit, int count) {
                    if (edit) {
                      ...
                      httpService.deleteData("kpi", count);
                      getData();
                    }
                  },
                  rowNumber: item["id"],
